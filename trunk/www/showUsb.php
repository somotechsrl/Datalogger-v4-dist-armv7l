...
<?
	...
	// exports
	if($dbid!="" && $mbid!="" && $date!="")
		echo "<a class='btn' href='/ExportDb.php?drv=$dbid&mbid=$mbid&date=$date'>Export CSV</a>";

	
	// gets filtered data
	if(isset($_GET["drv"])) {
		system(sprintf("sudo $scripts/dbDrvHtml '%s' '%s' '%s'",$dbid,$mbid,$date));
		}
?>
</form>
</body>
